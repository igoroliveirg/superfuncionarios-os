...
function apiMiddleware() {
  // registra uma rota POST que lê {url} e responde com fn(url) em JSON
  const route = (server, path, load, onError) => {
  ...
  return {
    ...
    configureServer(server) {
      ...
      route(server, '/api/identify', async () => (await import('./server/identify.mjs')).identify,
        (e) => ({ niche: 'generico', error: String(e?.message || e) }))
      route(server, '/api/generate', async () => (await import('./server/generate.mjs')).generatePack,
